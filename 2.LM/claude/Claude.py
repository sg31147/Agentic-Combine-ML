...
def predict_icd10_for_note(discharge_note):
    """Predict ICD-10 codes for a single discharge note using Claude Code"""

    system_content = """You are a medical coding specialist. Analyze discharge summaries and predict ICD-10 codes.
    
INSTRUCTIONS:
1. Return relevant codes
2. Use standard ICD-10 format (e.g., I50.9, N18.6)
3. Provide concise descriptions
4. Include probability scores (0.0-1.0)

OUTPUT FORMAT (JSON):
{
"E11.9": "Type 2 diabetes without complications (0.95)",
"I25.9": "Chronic ischemic heart disease (0.87)",
"N18.6": "End stage renal disease (0.82)"
}"""
...
def process_row(row_data):
    ...
    index, discharge_note = row_data
    try:
        prediction = predict_icd10_for_note(discharge_note)
...
def main(input_file='Claude.csv', output_file='Claude.csv', num_processes=None, start_row=None, stop_row=None):
    try:
        ...
        row_data = [(index, row['text']) for index, row in df_subset.iterrows()]
        ...
        print("Error: Column 'Discharge Summary Note' not found in CSV file.")
    except Exception as e:
        print(f"Error: {str(e)}")
